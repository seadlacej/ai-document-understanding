...
export async function processJob(jobId: string) {
  ...
  try {
    ...
    const uploadPath = path.join(UPLOADS_DIR, path.basename(fileRecord.originalName));
    await fs.copyFile(fileRecord.savedPath, uploadPath);
    
    // Run the analysis script
    console.log(`Starting analysis for job ${jobId}: ${fileRecord.originalName}`);
    ...
    const jobOutputDir = outputDirs
      .filter(dir => dir.includes(path.basename(fileRecord.originalName, '.pptx')))
      .sort()
      .pop();
    ...
    // Create zip file
    const zipFileName = `${jobId}_${path.basename(fileRecord.originalName, '.pptx')}.zip`;
